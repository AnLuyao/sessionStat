...
object SessionStat {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // (2589e32931384ed096ea284431e48170,
  ...
  def getTop10ActiveSession(sparkSession: SparkSession, taskUUID: String, top10Category: Array[(SortedKey, String)], sessionId2FilteredActionRDD: RDD[(String, UserVisitAction)]): Unit = {
    // sparkSession.sparkContext.makeRDD(top10Category) -> RDD[(sortKey, fullInfo)]
    // top10CategoryRDD: (cid, cid)
    ...
    // sessionId2FilteredActionRDD : (sid, aciton)
    ...
    // cid2ActionRDD.join(top10CategoryRDD)已经只剩下点击过Top10品类的action数据
    // map: 进行格式转换
    ...
    //第二种思路：
    // 过滤， 得到点击过TOP10热门品类的所有的action
    // 组合key：  categoryId + sessionId, 1
    // reducebykey：   (categoryId + sessionId,count)   sessionId对应的session对于categoryId的点击次数
    // 拆分key： categoryId + sessionId -> categoryId, sessionId+count
    // groupbykey: (cagoryId, iterable[session+count, session+count])
  }
  ...
  def getTop10PopularCategories(sparkSession: SparkSession, taskUUID: String, sessionId2FilteredActionRDD: RDD[(String, UserVisitAction)]) = {
    // 首先得到所有被点击、下单、付款过的品类
  ...
  def getFullInfoCount(categoryId2CidRDD: RDD[(Long, Long)], clickCount: RDD[(Long, Long)], orderCount: RDD[(Long, Long)], payCount: RDD[(Long, Long)]) = {
    // (cid, (cid, clickCount)
  ...
  def sessionRandomExtract(sparkSession: SparkSession, taskUUID: String, sessionId2FullInfoRDD: RDD[(String, String)]): Unit = {
    // 现在sessionId2FullInfoRDD它的key为sessionId，我们要把key转换为时间
    ...
    def generateRandomIndex(sessionExtractCountPerDay: Long, sessionDayCount: Long, hourCountMap: mutable.HashMap[String, Long], hourIndexMap: mutable.HashMap[String, mutable.ListBuffer[Int]]): Unit = {
  ...
  def getFilterRDD(sparkSession: SparkSession, taskParm: JSONObject, sessionStatisticAccumulator: SessionStatAccumulator, sessionId2FullInfoRDD: RDD[(String, String)]) = {
    // 从taskParam提取限制条件
  ...
  def getSessionFullInfo(sparkSession: SparkSession,
    ...
    val userId2AggrInfoRDD = sessionId2GroupRDD.map {
      case (sessionId, iterableAction) =>
        ...
        // if(searchKeywords.toString.endsWith(","))
        // val searchKW1 = searchKeywords.toString.substring(0, searchKeywords.toString.length -1)
    ...
    import sparkSession.implicits._
    // DF[Row] -> RDD[Row]  getAs[]() / getLong()
    // DF[Row] -> as[UserInfo] -> RDD[UserInfo] -> .userId
